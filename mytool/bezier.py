...
class BezierTrajectory:
    ...
    def simulation(cls, start, end, le=1, deviation=0, bias=0.5):
        ...
        return {
            "equation": cls._bztsg(dataTrajectoryList),
            "P": np.array(dataTrajectoryList),
        }

    @classmethod
    def trackArray(
        cls, start, end, numberList, le=1, deviation=0, bias=0.5, type=0, cbb=0, yhh=10
    ):
        """
    ...
    def _get_bezier_params(cls, start_x: int, start_y: int, end_x: int, end_y: int):
        ...
        bezier_num = (
            int(abs(end_x - start_x) / 4)
            if abs(end_x - start_x) > abs(end_y - start_y)
            else int(abs(end_y - start_y) / 4)
        )
    ...
    def move_by_bezier(cls, start_x: int, start_y: int, end_x: int, end_y: int):
        bezier_num, bezier_level, bezier_type = cls._get_bezier_params(
            start_x, start_y, end_x, end_y
        )
        # print(bezier_num, bezier_level, bezier_type)
        move_list = BezierTrajectory.trackArray(
            start=(start_x, start_y),
            end=(end_x, end_y),
            numberList=bezier_num,
            le=bezier_level,
            deviation=10,
            bias=0.5,
            type=bezier_type[0],
        ...
        type = "sp" if abs(end_x - start_x) > abs(end_y - start_y) else "sz"
        match type:
            case "sp":
                if start_x > end_x:
                    return sorted(move_list, key=lambda x: x[0], reverse=True)
                else:
                    return sorted(move_list, key=lambda x: x[0], reverse=False)
            case "sz":
                if start_y > end_y:
                    return sorted(move_list, key=lambda x: x[1], reverse=True)
                else:
                    return sorted(move_list, key=lambda x: x[1], reverse=False)

        pass


if __name__ == "__main__":
    print(BezierTrajectory.move_by_bezier(1177, 166, 263, 195))
