...
class Taigu(Mouse,ImgRec):
    ...
    def get_num(self,result:list[image],mode:str):
        ...
        for img in result:
            ...
            if res:=Ocr().ocr([832,423,946,446]):
                if '勾玉' in res[0]:
                    num=int(''.join(char for char in res[0] if char.isdigit())) if res[1]>0.7 else 0
                    log.info(f'get_num:{num}')
                else:num=0
                match mode:
                    case 'find':
                        self.temp_max=max(self.temp_max,num)
                        if self.temp_max==76:self.shoot_target()
                    case 'shot':
                        if num>=self.max:self.shoot_target();return
        return True
    def refind_max(self):
        ...
        for _ in range(2):
            for i in range(4):
                ...
                if res:=self.match_duo_img(tg_6x,):
                    result=res
                if res:=self.match_duo_img(tg_5x,):
                    result+=res
                self.get_num(result,'shot')
                self.random_click([428,192,461,249])#428,192,461,249
                sleep(0.5)
                self.mouse_scroll(('down',5+i%2),*rect(283,177,578,584).random_point)
                sleep(0.5)
                result=[]
    ...
    def find_max(self):
        ...
        for i in range(4):
            ...
            self.random_click([428,192,461,249])#428,192,461,249
            sleep(0.5)
            self.mouse_scroll(('down',5+i%2),*rect(370,268,473,491).random_point)
            sleep(0.5)
            result=[]
        ...
        match res:
            case None:
                ...
                if time:=Ocr().ocr([1145,123,1236,144])[0]:
                    ...
                    self.next_time=int(time.split(':')[0])*60+int(time.split(':')[1])+1
                    self.switch=True
    ...
    def run(self):
        ...
        match curr_ui:
            case 'jy_out':self.jy_match()
            case 'jy_ui':
                self.find_max()
            case _:log.error("No match ui")
...
def taigu_run():
    if client.get_handle('MuMu模拟器12'):#已经启动
        ...
        sleep(10)
    taigu=Taigu()
    while not taigu.page_switch.switch_to('jy_out'):sleep(1)
    ...
    if hasattr(client,'kill_app'):
        ...
        if client.kill_app:client.stop();log.info('MuMu模拟器12 is stoped')
